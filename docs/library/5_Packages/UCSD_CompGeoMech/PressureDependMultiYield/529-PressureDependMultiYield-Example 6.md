...
set gamma 0.600 ;# Newmark integration parameter</p>
...
material tag 
set solid1 10 ;# solid material tag</p>
...
<p>set bUnitWeightX [expr ($rhoS-0.0)*9.81*sin($inclination/180.0*$pi)*$densityMult] ;# Total unit
weight in X direction set bUnitWeightY 0.0 ;# buoyant unit weight in Y
direction set bUnitWeightZ [expr -($rhoS-$rhoF)*9.81*cos($inclination/180.0*$pi)] ;# buoyant unit weight
in Z direction</p>
...
set nodeList {} 
for {set i 1} {$i &lt;= 8 } {incr i 1} { lappend nodeList $i }
set elementList {} 
for {set i 1} {$i &lt;= 1 } {incr i 1} { lappend elementList $i }

# GRAVITY APPLICATION (elastic behavior)
<li>create the SOE, ConstraintHandler, Integrator, Algorithm and Numberer</li>
...
<p>fs=[0.5, 0.2, 4, 6];
accMul = 4;</p>
<p>%integration point 1 p-q po=(s1(:,2)+s1(:,3)+s1(:,4))/3;
for
...
clf; %integration
point 1 stress-strain subplot(2,1,1), plot(e1(:,7),s1(:,7),'r');
title
...
yLabel('Shear
stress \tau_x_y (kPa)');</p>
<p>subplot(2,1,2), plot(-po,qo,'r');
title ('Integration point 1
confinement p VS. deviatoric q relation');
xLabel('confinement p
(kPa)');
yLabel('q (kPa)');
set(gcf,'paperposition',fs);
saveas(gcf,'SS_PQ1','jpg');</p>
<p>%integration point 5 p-q 
po=(s5(:,2)+s5(:,3)+s5(:,4))/3;
for i=1:size(s5,1) 
...
figure(2);
clf; %node 3 displacement relative to node 1
