...
$B 31.4 .1 80 0.5 \
        ...
        1e-6 0.999 3e-6
0.995 1e-5 0.99 3e-5 0.96 1e-4 0.85 3e-4 0.64 \
        1e-3 0.37 3e-3 0.18 1e-2
...
PressureIndependMultiYield 1 2 $mass 4.e4 2.e5 20 .1 0 100 0. -11\
        1e-6
0.999 3e-6 0.995 1e-5 0.99 3e-5 0.96 1e-4 0.85 3e-4 0.64 \
        1e-3 0.37
...
0.3 $mass set gravY [expr -9.81*$mass] ;#gravity set gravX [expr -$gravY*$loadBias] } }</p>
...
system ProfileSPD
<p>test NormDispIncr 1.e-5 10 0 algorithm ModifiedNewton
constraints Transformation
 integrator LoadControl 1 1 1 1 numberer RCM</p>

# create the Analysis

analysis Static analyze 2
...
eval "recorder Node -file disp -time -node $nodeList -dof 1 2 -dT $deltaT disp"der Node -file acc -time -node $nodeList -dof 1 2 -dT $deltaT
accel"</p>
...
$InitStiffnessProportionalDamping 0.0 integrator Newmark $gamma [expr pow($gamma+0.5, 2)/4] analysis VariableTransient</p>
